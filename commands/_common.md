...
**所有标记为「必须确认」的操作，MUST 使用以下模式执行确认，不得跳过、不得默认选择、不得自行判断用户意图：**

### 确认级别

| 级别 | 适用场景 | 要求 |
|------|---------|------|
| 🔴 强制确认 | 不可逆操作（归档、删除、覆盖、git commit） | 展示完整摘要后，**必须停下来等待用户明确回复 y 才可继续**，用户未回复前禁止执行任何后续操作 |
| 🟡 标准确认 | 写入操作（编辑文档、写入方案、开始编码） | 展示变更预览后等待用户确认，**用户回复前禁止执行 Write/Edit 或修改代码** |
| 🟢 轻量确认 | 可选操作（创建测试、查看建议） | 询问后根据回复决定 |

### 强制确认的执行格式

当命令文件中标注了 `⚠️🔴 强制确认` 或 `⚠️🟡 标准确认` 时，MUST 按以下格式输出并**立即停止当前回复，等待用户回复**：
...
1. **禁止在同一条消息中既展示确认提示又继续执行后续步骤** — 确认提示必须是当前回复的最后内容
2. **禁止将 y 作为默认值** — 用户必须显式回复
3. **禁止根据上下文推断用户意图来跳过确认** — 即使用户之前表达过意图，仍需确认
4. **禁止合并多个确认** — 每个确认点独立执行
