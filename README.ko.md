...
| 플러그인 | 설명 | 버전 |
|---------|------|------|
| **req** | 요구사항 전체 워크플로우 — 분석부터 아카이브까지 | v3.0.1 |
| **pm** | 프로젝트 관리 도우미 — 주간/월간 리포트, 통계, 리스크 스캔, 기획안 | v0.1.0 |
| **api** | API 연동 — Swagger 파싱, 필드 매핑, 코드 생성 | v0.2.0 |
...
> **aiforge 에서 업그레이드**: marketplace 이름이 `aiforge` 에서 `devflow` 로 변경되었습니다. 기존 사용자는 재설치가 필요합니다:
> ```bash
